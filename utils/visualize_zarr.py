...
class ZarrVisualizer:
    ...
    def setup_visualization(self):
        ...
        self.vis.register_key_callback(
            ord('N'), lambda vis: self.next_sequence())
        self.vis.register_key_callback(
            ord('P'), lambda vis: self.prev_sequence())
        self.vis.register_key_callback(
            ord('+'), lambda vis: self.adjust_point_size(0.5))
        self.vis.register_key_callback(
            ord('-'), lambda vis: self.adjust_point_size(-0.5))
    ...
    def reset_view(self):
        ...
        # Set view parameters similar to viz_flow_3d_data.py and visualize_3d_rgbdk_or_obj_or_pcd.py
        ...
        # Camera direction from origin to center
        view_control.set_front(-scene_center)
    ...
    def update_frame(self):
        ...
        if len(hand_vertices) > 0:  # Only update if we have vertices
            ...
            self.hand_pcd.points = o3d.utility.Vector3dVector(
                scaled_hand_vertices)
    ...
    def next_sequence(self):
        ...
        self.current_sequence_idx = (
            self.current_sequence_idx + 1) % len(self.sequences)
        self.load_sequence()
        self.update_frame()
        self.reset_view()
        return False
    ...
    def run(self):
        ...
        while True:
            ...
            if not self.paused and (now - last_update) > 0.033:  # ~30 FPS
                ...
                last_update = now
                print(
                    f"\rCurrent frame: {self.current_frame}/{self.num_frames-1}", end="")
...
def main():
    parser = argparse.ArgumentParser(
        description='Visualize WiLoR hand and scene point clouds')
